...
for (cr in change_rate) {
  for (cl in 1:1) { # classificadores
      for(algorithm in 6:6){ # Algoritmos
        for(i in 1:31) { # Bases
          ...
          for(j in 1:5) {
            ...
            for (fold in 1:length(folds)) {
              ...
              partial_acc <- training(cl,base_original,base_rotulados_ini,base_teste,as.character(algorithm))
