...
if(c==1){ #NAIVE BAYES
  ...
  if (t==1){ #TAXA INICIAL 0.9
    #chamada da funcao que implementa o metodo flexCon_C2 usando naive
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner("naiveBayes", list(4)),'func',0.9,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }else if (t==2){ #TAXA INICIAL 0.95
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner("naiveBayes", list(4)),'func',0.95,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }
}
...
if(c==3){ #RIPPER
  ...
  if (t==1){
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner('JRip',list()),'f2',0.9,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }else if (t==2){
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner('JRip',list()),'f2',0.95,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }  
}
if(c==4){ #IBK
  ...
  if (t==1){
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner('IBk',list(control = Weka_control(K=15, X=TRUE))),'f2',0.9,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }else if (t==2){
    CT<- coTrainFlexCon_C2(as.formula(paste(classe,'~', '.')), base_treino_self_training,learner('IBk',list(control = Weka_control(K=15, X=TRUE))),'f2',0.95,100,1,TRUE,qtd_exem_menor_classe, limiar = acc_sup_3, stdNB)
  }  
}
